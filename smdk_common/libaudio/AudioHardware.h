...
#define AUDIO_HW_OUT_PERIOD_SZ 2048 /* 1024 */	//org
//#define AUDIO_HW_OUT_PERIOD_SZ 1024	//yyd- 120130
...
#define AUDIO_HW_IN_PERIOD_SZ 2048	//org
//#define AUDIO_HW_IN_PERIOD_SZ 1024	//yyd- 120130
...
#define AUDIO_DEVICE_OUT_ALL_SCO (AudioSystem::DEVICE_OUT_BLUETOOTH_SCO |\
...
public:
    ...
    const AudioMixer *getOutputRouteFromDevice(uint32_t device);
    const AudioMixer *getInputRouteFromDevice(uint32_t device);
    const AudioMixer *getVoiceRouteFromDevice(uint32_t device);

    status_t setIncallPath_l(uint32_t device);

    int setCallVmOperation_l(int order);
    ...
    struct pcm *openVoicePcmOut_l();
    void closeVoicePcmOut_l();
...
private:
    ...
    Mutex           mLock;
    struct pcm*     mPcm;
    struct pcm*     mVoicePcm;
    struct mixer*   mMixer;
    uint32_t        mPcmOpenCnt;
    uint32_t        mVoicePcmOpenCnt;
    uint32_t        mMixerOpenCnt;
    bool            mInCallAudioMode;
    float           mVoiceVol;
    uint32_t        mVoiceDevice;
    ...
    int             (*setCallVmOperation)(HRilClient, int);
    void            loadRILD(void);
    status_t        connectRILDIfRequired(void);
    struct echo_reference_itfe *mEchoReference;

    //Add by CYIT 20130729----start-----
//    void            openAudioATChannel();
    static void*    ThreadWrapper(void *me);
    void            threadFunc();
    sp<SecRil>      mSecRil;
    pthread_t       mOpenATInterfaceThread;
    int             mATChannelOk;
    //Add by CYIT 20130729-----end------
    ...
    class AudioStreamInALSA : public AudioStreamIn, public RefBase
    {
     ...
     public:
        ...
        virtual int format() const { return mFormat; }//modified by cyit 20130106
