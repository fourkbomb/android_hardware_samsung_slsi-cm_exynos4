...
inline int EXYNOS_MIN(int x, int y)
{
    return ((x < y) ? x : y);
}

inline int EXYNOS_MAX(int x, int y)
{
    return ((x > y) ? x : y);
}
...
#define EXYNOS_HWC_LOG_TAG     "EXYNOSHWC_LOG"

#ifdef HWC_DEBUG
#define EXYNOS_HWC_Log(a, ...)    ((void)_EXYNOS_HWC_Log(a, EXYNOS_HWC_LOG_TAG, __VA_ARGS__))
#else
#define EXYNOS_HWC_Log(a, ...)                                         \
    do {                                                            \
        if (a == HWC_LOG_ERROR)                                     \
            ((void)_EXYNOS_HWC_Log(a, EXYNOS_HWC_LOG_TAG, __VA_ARGS__)); \
    } while (0)
#endif

extern void _EXYNOS_HWC_Log(HWC_LOG_LEVEL logLevel, const char *tag, const char *msg, ...);
...
#endif /* ANDROID_EXYNOS_HWC_UTILS_H_*/
