...
int window_open(struct hwc_win_info_t *win, int id)
{
    ...
    case 1:
        ...
        break;
    default:
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::id(%d) is weird", __func__, id);
        goto error;
}
...
//    snprintf(name, 64, device_template, id + 1);
    ...
    if (win->fd <= 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::Failed to open window device (%s) : %s",
    ...
    if (ioctl(win->fd, S3CFB_SET_VSYNC_INT, &vsync) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::S3CFB_SET_VSYNC_INT fail", __func__);
        goto error;
    }
#endif
...
int window_set_pos(struct hwc_win_info_t *win)
{
    ...
    /*  //wjj close for video flicker until find real reason. 130201
    if (window_hide(win) != 0)
        return -1;
    */

    EXYNOS_HWC_Log(HWC_LOG_DEBUG, "%s:: x(%d), y(%d)",
    ...
    if (ioctl(win->fd, FBIOPUT_VSCREENINFO, &(win->var_info)) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::FBIOPUT_VSCREENINFO(%d, %d) fail",
    ...
    if (ioctl(win->fd, S3CFB_WIN_POSITION, &window) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::S3CFB_WIN_POSITION(%d, %d) fail",
...
int window_get_info(struct hwc_win_info_t *win, int win_num)
{
    ...
    if (ioctl(win->fd, FBIOGET_FSCREENINFO, &win->fix_info) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "FBIOGET_FSCREENINFO failed : %s",
                strerror(errno));
        goto error;
    }
    ...
    if (ioctl(win->fd, S3CFB_GET_ION_USER_HANDLE, &ion_handle) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Get fb ion client is failed\n");
        return -1;
    }
...
#endif
        EXYNOS_HWC_Log(HWC_LOG_DEBUG, "%s::win-%d add[%d]  %x ",
...
int window_pan_display(struct hwc_win_info_t *win)
{
    ...
    if (ioctl(win->fd, FBIO_WAITFORVSYNC, &fimd_num) < 0)
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::FBIO_WAITFORVSYNC fail(%s)",
    ...
    if (ioctl(win->fd, FBIOPAN_DISPLAY, lcd_info) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::FBIOPAN_DISPLAY(%d / %d / %d) fail(%s)",
            __func__,
            lcd_info->yres,
...
int window_show(struct hwc_win_info_t *win)
{
    if (win->power_state == 0) {
        if (ioctl(win->fd, FBIOBLANK, FB_BLANK_UNBLANK) < 0) {
            EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::FBIOBLANK failed : (%d:%s)",
...
int window_get_global_lcd_info(int fd, struct fb_var_screeninfo *lcd_info)
{
    if (ioctl(fd, FBIOGET_VSCREENINFO, lcd_info) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "FBIOGET_VSCREENINFO failed : %s",
                strerror(errno));
        return -1;
    }

    EXYNOS_HWC_Log(HWC_LOG_DEBUG, "%s:: Default LCD x(%d),y(%d)",
...
int fimc_v4l2_set_src(int fd, unsigned int hw_ver, s5p_fimc_img_info *src)
{
    ...
    if (ioctl(fd, VIDIOC_S_FMT, &fmt) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::VIDIOC_S_FMT failed : errno=%d (%s)"
    ...
    if (ioctl(fd, VIDIOC_S_CROP, &crop) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::Error in video VIDIOC_S_CROP :"
    ...
    if (ioctl(fd, VIDIOC_REQBUFS, &req) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::Error in VIDIOC_REQBUFS", __func__);
        return -1;
    }
        ...
        int rotation, int hflip, int vflip, unsigned int addr)
{
    ...
    if (ret < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::Error in video VIDIOC_S_FMT (%d)", __func__, ret);
        return -1;
    }
    ...
    if (ret < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::Error in video VIDIOC_S_CROP (%d)", __func__, ret);
        return -1;
    }
    ...
    if (ret < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::Error in VIDIOC_REQBUFS (%d)", __func__, ret);
        return -1;
    }
#else
    ...
    if (ret < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::Error in video VIDIOC_G_FBUF (%d)", __func__, ret);
        return -1;
    }
    ...
    if (ret < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::Error in video VIDIOC_S_FBUF (%d)", __func__, ret);
        return -1;
    }
...
int fimc_v4l2_stream_on(int fd, enum v4l2_buf_type type)
{
    if (-1 == ioctl(fd, VIDIOC_STREAMON, &type)) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Error in VIDIOC_STREAMON\n");
        return -1;
    }
...
int fimc_v4l2_queue(int fd, struct fimc_buf *fimc_buf, enum v4l2_buf_type type, int index)
{
    ...
    if (0 > ret) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Error in VIDIOC_QBUF : (%d)", ret);
        return -1;
    }
...
int fimc_v4l2_dequeue(int fd, struct fimc_buf *fimc_buf, enum v4l2_buf_type type)
{
    ...
    if (-1 == ioctl(fd, VIDIOC_DQBUF, &buf)) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Error in VIDIOC_DQBUF\n");
        return -1;
    }
...
int fimc_v4l2_stream_off(int fd, enum v4l2_buf_type type)
{
    if (-1 == ioctl(fd, VIDIOC_STREAMOFF, &type)) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Error in VIDIOC_STREAMOFF\n");
        return -1;
    }
...
int fimc_v4l2_clr_buf(int fd, enum v4l2_buf_type type)
{
    ...
    if (ioctl(fd, VIDIOC_REQBUFS, &req) == -1) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Error in VIDIOC_REQBUFS");
    }
...
int fimc_v4l2_S_ctrl(int fd)
{
    ...
    if (ioctl(fd, VIDIOC_S_CTRL, &vc) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Error in VIDIOC_S_CTRL");
        return -1;
    }
...
int fimc_handle_oneshot(int fd, struct fimc_buf *fimc_src_buf, struct fimc_buf *fimc_dst_buf)
{
    ...
    if (fimc_v4l2_queue(fd, fimc_src_buf, V4L2_BUF_TYPE_OUTPUT, 0) < 0) {
         EXYNOS_HWC_Log(HWC_LOG_ERROR, "Fail : SRC v4l2_queue()");
         return -1;
     }

    if (fimc_v4l2_queue(fd, fimc_dst_buf, V4L2_BUF_TYPE_CAPTURE, 0) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Fail : DST v4l2_queue()");
        return -2;
    }

    if (fimc_v4l2_stream_on(fd, V4L2_BUF_TYPE_OUTPUT) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Fail : SRC v4l2_stream_on()");
        return -3;
    }

    if (fimc_v4l2_stream_on(fd, V4L2_BUF_TYPE_CAPTURE) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Fail : DST v4l2_stream_on()");
        return -4;
    }
#else
    if (fimc_v4l2_stream_on(fd, V4L2_BUF_TYPE_OUTPUT) < 0) {
    ...
    if (fimc_v4l2_dequeue(fd, fimc_src_buf, V4L2_BUF_TYPE_OUTPUT) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Fail : SRC v4l2_dequeue()");
        return -6;
    }
#ifdef BOARD_USE_V4L2_ION
    if (fimc_v4l2_dequeue(fd, fimc_dst_buf, V4L2_BUF_TYPE_CAPTURE) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Fail : DST v4l2_dequeue()");
        return -7;
    }
#endif
STREAM_OFF:
    if (fimc_v4l2_stream_off(fd, V4L2_BUF_TYPE_OUTPUT) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Fail : SRC  v4l2_stream_off()");
        return -8;
    }
#ifdef BOARD_USE_V4L2_ION 
    if (fimc_v4l2_stream_off(fd, V4L2_BUF_TYPE_CAPTURE) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Fail : DST  v4l2_stream_off()");
        return -9;
    }
#endif
    if (fimc_v4l2_clr_buf(fd, V4L2_BUF_TYPE_OUTPUT) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Fail : SRC v4l2_clr_buf()");
        return -10;
    }
#ifdef BOARD_USE_V4L2_ION
    if (fimc_v4l2_clr_buf(fd, V4L2_BUF_TYPE_CAPTURE)< 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "Fail : DST v4l2_clr_buf()");
        return -11;
    }
#endif
    return 0;
...
*/
        ...
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "use default memcpy instead of memcpy_rect");
        return -1;
    }
//#define CHECK_PERF
#ifdef CHECK_PERF
    ...
    EXYNOS_HWC_Log(HWC_LOG_ERROR, "[COPY]=%d,",(end.tv_sec - start.tv_sec)*1000+(end.tv_usec - start.tv_usec)/1000);
#endif
...
/*****************************************************************************/
static int get_src_phys_addr(struct exynos4_hwc_composer_device_1_t *ctx,
        sec_img *src_img, sec_rect *src_rect)
{
    ...
    if (0 == src_img->base && !(src_img->usage & GRALLOC_USAGE_HW_FIMC1)) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s invalid src image base\n", __func__);
        return 0;
    }
    ...
    case HWC_UNKNOWN_MEM_TYPE:
        ...
        case HAL_PIXEL_FORMAT_CUSTOM_YCrCb_422_SP:
            ...
            if (0 == src_phys_addr) {
                EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s address error "
        ...
        default:
            ...
            if (src_img->usage & GRALLOC_USAGE_HW_FIMC1) {
                ...
                EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::\nformat = 0x%x : Not "
...
static int runFimcCore(struct exynos4_hwc_composer_device_1_t *ctx,
        ...
        uint32_t dst_color_space, int transform)
{
   ...
   /* 3. Set configuration related to destination (DMA-OUT)
    ...
    if (fimc_v4l2_set_dst(fimc->dev_fd, &params->dst, rotate_value, hflip, vflip, dst_phys_addr) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "fimc_v4l2_set_dst is failed\n");
        return -1;
    }
   ...
   /* 4. Set configuration related to source (DMA-INPUT)
    ...
    if (fimc_v4l2_set_src(fimc->dev_fd, fimc->hw_ver, &params->src) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "fimc_v4l2_set_src is failed\n");
        return -1;
    }
#endif
    ...
    if (ret < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR,"fimc_handle_oneshot = %d\n",ret);
        if (ret == -2) {
...
int createFimc(s5p_fimc_t *fimc)
{
    ...
    if (fimc->dev_fd <= 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::Post processor open error (%d)",
                __func__, errno);
        goto err;
    }
    ...
    if (ioctl(fimc->dev_fd, VIDIOC_QUERYCAP, &cap) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "VIDIOC_QUERYCAP failed");
        goto err;
    }

    if (!(cap.capabilities & V4L2_CAP_STREAMING)) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%d has no streaming support", fimc->dev_fd);
        goto err;
    }

    if (!(cap.capabilities & V4L2_CAP_VIDEO_OUTPUT)) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%d is no video output", fimc->dev_fd);
        goto err;
    }
    ...
    if (ioctl(fimc->dev_fd, VIDIOC_G_FMT, &fmt) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::Error in video VIDIOC_G_FMT", __func__);
        goto err;
    }
    ...
    if (ioctl(fimc->dev_fd, VIDIOC_G_CTRL, &vc) < 0) {
        EXYNOS_HWC_Log(HWC_LOG_ERROR, "%s::Error in video VIDIOC_G_CTRL", __func__);
        goto err;
    }
