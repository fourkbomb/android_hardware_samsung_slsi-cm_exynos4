...
#define V4L2_HFLIP                 V4L2_CID_HFLIP //yqf test
#define V4L2_VFLIP                 V4L2_CID_VFLIP
#else
...
//added yqf, test
bool SecFimc::setFlipVal(unsigned int flipVal)
{
    struct v4l2_control vc;
...
if(flipVal==1){
    	if (fimc_v4l2_s_ctrl(mFd, V4L2_HFLIP, 1) < 0) {
        	LOGE("%s::fimc_v4l2_s_ctrl(V4L2_ROTATE) failed", __func__);
        	return false;
    	}
}
else if(flipVal==2){
 	if (fimc_v4l2_s_ctrl(mFd, V4L2_VFLIP, 1) < 0) {
        	LOGE("%s::fimc_v4l2_s_ctrl(V4L2_ROTATE) failed", __func__);
        	return false;
    	}
}else{
 	if (fimc_v4l2_s_ctrl(mFd, V4L2_HFLIP, 0) < 0) {
        	LOGE("%s::fimc_v4l2_s_ctrl(V4L2_ROTATE) failed", __func__);
        	return false;
    	}
 	if (fimc_v4l2_s_ctrl(mFd, V4L2_VFLIP, 0) < 0) {
        	LOGE("%s::fimc_v4l2_s_ctrl(V4L2_ROTATE) failed", __func__);
        	return false;
    	}
}
