...
void SecHdmiClient::setHdmiCableStatus(int status)
{
    ...
    if (g_SecTVOutService != 0){
   	//LOGD("lt::%s::setHdmiCableStatus::status=%d",__func__, status);//lt,debug
...
//yqf test
void SecHdmiClient::setHdmiFlip(int flipVal, uint32_t hwcLayer)
{
    //LOGD("%s HDMI ROTATE: %d\n", __func__, rotVal);

    if (g_SecTVOutService != 0)
        g_SecTVOutService->setHdmiFlip(flipVal, hwcLayer);
}
//yqf test
void SecHdmiClient::saveHdmiRotate(int rotVal)
{
    //LOGD("%s HDMI ROTATE: %d\n", __func__, rotVal);

    if (g_SecTVOutService != 0)
        g_SecTVOutService->saveHdmiRotate(rotVal);
}
