...
//#define CHECK_UI_TIME
    ...
    class SecTVOutService : public BBinder
    {
        public :
	     ...
	     virtual void                        setHdmiFlip(uint32_t flipVal, uint32_t hwcLayer); //added yqf test
	     virtual void                        saveHdmiRotate(uint32_t rotVal);  //added yqf test
        ...
        private:
            ...
            uint32_t                    mHwcLayer;
	     uint32_t	              globalRotate;  //added yqf
	     uint32_t 			width;                     
	     uint32_t 			height; 
	     uint32_t 			colorformat; 
            uint32_t 			phyYAddr;
            uint32_t 			phyCbAddr;
            uint32_t 			phyCrAddr;        //yqf, added end
    };
